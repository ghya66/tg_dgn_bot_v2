...
class TestPremiumSecurity:
    ...
    @pytest.fixture
    def mock_db_context(self, test_db):
        """创建 mock db context"""
        @contextmanager
    ...
    @pytest.fixture
    def security_service(self, mock_db_context):
        """创建安全服务实例"""
    ...
    @pytest.mark.asyncio
    async def test_daily_purchase_limit(self, test_db, mock_db_context):
        """测试每日购买限制"""
        user_id = 123456
    ...
    @pytest.mark.asyncio
    async def test_order_interval_check(self, test_db, mock_db_context):
        """测试订单间隔检查"""
        user_id = 123457
    ...
    @pytest.mark.asyncio
    async def test_blacklist(self, mock_db_context):
        """测试黑名单功能"""
        user_id = 999999
        ...
        with patch('src.modules.premium.security.get_db_context', mock_db_context):
            ...
            # 初始不在黑名单
            assert service.is_blacklisted(user_id) is False

            # 添加到黑名单
            await service.add_to_blacklist(user_id, "测试原因")
            assert service.is_blacklisted(user_id) is True
            ...
            # 从黑名单移除
            await service.remove_from_blacklist(user_id)
            assert service.is_blacklisted(user_id) is False

    @pytest.mark.asyncio
    async def test_recipient_limits(self, test_db, mock_db_context):
        """测试收件人限制"""
        ...
        with patch('src.modules.premium.security.get_db_context', mock_db_context):
            ...
            # 检查限制
            result = await service.check_recipient_limits(recipient_username)
            assert result["allowed"] is False
            assert "上限" in result["reason"]

    @pytest.mark.asyncio
    async def test_suspicious_behavior_detection(self, test_db, mock_db_context):
        """测试异常行为检测"""
        user_id = 888888
        ...
        with patch('src.modules.premium.security.get_db_context', mock_db_context):
            ...
            # 检测异常
            result = await service.detect_suspicious_behavior(user_id)
            assert result["suspicious"] is True
            assert result["risk_score"] > 50
            assert result["reason"] is not None

    @pytest.mark.asyncio
    async def test_validate_order(self, mock_db_context):
        """测试综合订单验证"""
        user_id = 777777
