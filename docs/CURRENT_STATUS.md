...
```
总计: 761 passed, 1 failed, 2 skipped ✅
运行时间: ~5分钟32秒
失败: test_concurrent_order_creation (并发压力测试，已知问题)
```
...
| 日期 | 更新内容 |
|------|---------|
| 2025-12-07 | 数据库配置统一：合并 `./tg_bot.db` 和 `./data/tg_bot.db`，统一使用 `./data/tg_bot.db` |
...
- ✅ 合并 `./tg_bot.db` 和 `./data/tg_bot.db` 两个数据库文件
- ✅ 统一使用 `./data/tg_bot.db` 作为默认数据库路径
- ✅ 修改 `src/database.py` 第14行默认值
- ✅ 修改 `alembic.ini` 第17行迁移配置
- ✅ 所有部署方式（本地/Docker/Render）均兼容
