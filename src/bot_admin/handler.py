...
from .middleware import get_owner_id, owner_only
...
class AdminHandler:
    ...
    async def admin_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 记录审计日志
        audit_logger.log(admin_id=user_id, action="open_admin_panel", details="打开管理面板")
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Premium 价格编辑
            ...
            return EDITING_PREMIUM_3 if months == "3" else (EDITING_PREMIUM_6 if months == "6" else EDITING_PREMIUM_12)
        ...
        # 能量价格编辑
            ...
            await query.edit_message_text(
                f"⚡ <b>修改{name}价格</b>\n\n当前价格：{current} TRX\n\n请输入新价格（例如：3.5）：", parse_mode="HTML"
            )

            return (
                EDITING_ENERGY_SMALL
                if energy_type == "small"
                else (EDITING_ENERGY_LARGE if energy_type == "large" else EDITING_ENERGY_PACKAGE)
            )
    ...
    async def _show_main_menu(self, query):
        """显示主菜单"""
        await query.edit_message_text(
            "🔐 <b>管理员面板</b>\n\n请选择要执行的操作：", reply_markup=self.menus.main_menu(), parse_mode="HTML"
        )
    ...
    async def _show_stats(self, query, context):
        ...
        await query.edit_message_text(text, reply_markup=self.menus.back_to_main(), parse_mode="HTML")

        # 记录审计
        audit_logger.log(admin_id=query.from_user.id, action="view_stats", details="查看统计数据")
    ...
    async def _show_price_menu(self, query):
        """显示价格配置菜单"""
        await query.edit_message_text(
            "💰 <b>价格配置</b>\n\n请选择要配置的项目：", reply_markup=self.menus.price_menu(), parse_mode="HTML"
        )
    ...
    async def _show_premium_price(self, query):
        ...
        await query.edit_message_text(text, reply_markup=self.menus.premium_price_menu(), parse_mode="HTML")
    ...
    async def _show_energy_price(self, query):
        ...
        await query.edit_message_text(text, reply_markup=self.menus.energy_price_menu(), parse_mode="HTML")
    ...
    async def handle_premium_price_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, months: str):
        ...
        try:
            ...
            if new_price <= 0:
                ...
                return (
                    EDITING_PREMIUM_3 if months == "3" else (EDITING_PREMIUM_6 if months == "6" else EDITING_PREMIUM_12)
                )

            # 保存配置
            key = f"premium_{months}_months"
            success = config_manager.set_price(key, new_price, update.effective_user.id, f"Premium {months}个月价格")

            if success:
                # 记录审计
                audit_logger.log(
                    admin_id=update.effective_user.id, action="update_price", target=key, details=f"修改为 ${new_price}"
                )
    ...
    async def handle_trx_rate_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # 保存配置
            success = config_manager.set_price("trx_exchange_rate", new_rate, update.effective_user.id, "TRX 兑换汇率")
    ...
    async def handle_energy_price_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, energy_type: str):
        ...
        try:
            ...
            name, key = type_map[energy_type]
            success = config_manager.set_price(key, new_price, update.effective_user.id, f"{name}价格(TRX)")
    ...
    async def _show_content_menu(self, query):
        """显示文案配置菜单"""
        await query.edit_message_text(
            "📝 <b>文案配置</b>\n\n请选择要配置的项目：", reply_markup=self.menus.content_menu(), parse_mode="HTML"
        )
    ...
    async def handle_welcome_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if success:
            ...
            await update.message.reply_text(
                "✅ <b>欢迎语已更新</b>\n\n生效时间：立即\n\n使用 /admin 返回管理面板", parse_mode="HTML"
            )
        else:
            await update.message.reply_text("❌ 保存失败，请稍后重试")
    ...
    async def _show_settings_menu(self, query):
        """显示系统设置菜单"""
        await query.edit_message_text(
            "⚙️ <b>系统设置</b>\n\n请选择要配置的项目：", reply_markup=self.menus.settings_menu(), parse_mode="HTML"
        )
    ...
    async def _clear_cache(self, query):
        ...
        try:
            ...
            audit_logger.log(admin_id=query.from_user.id, action="clear_cache", details="清理 Redis 缓存")
            ...
            await query.edit_message_text(
                f"❌ 清理失败：{e!s}", reply_markup=self.menus.back_to_main(), parse_mode="HTML"
            )
    ...
    async def _show_system_status(self, query):
        ...
        try:
            ...
            await query.edit_message_text(text_msg, reply_markup=self.menus.back_to_main(), parse_mode="HTML")

        except Exception as e:
            logger.error(f"Failed to check system status: {e}")
            await query.edit_message_text(
                f"❌ 检查失败：{e!s}", reply_markup=self.menus.back_to_main(), parse_mode="HTML"
            )
    ...
    def get_conversation_handler(self) -> ConversationHandler:
        ...
        return ConversationHandler(
            ...
            states={
                EDITING_PREMIUM_3: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_premium_price_input(u, c, "3")
                    )
                ],
                EDITING_PREMIUM_6: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_premium_price_input(u, c, "6")
                    )
                ],
                EDITING_PREMIUM_12: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_premium_price_input(u, c, "12")
                    )
                ],
                EDITING_TRX_RATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_trx_rate_input)],
                EDITING_ENERGY_SMALL: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_energy_price_input(u, c, "small")
                    )
                ],
                EDITING_ENERGY_LARGE: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_energy_price_input(u, c, "large")
                    )
                ],
                EDITING_ENERGY_PACKAGE: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, lambda u, c: self.handle_energy_price_input(u, c, "package")
                    )
                ],
                EDITING_TIMEOUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_timeout_input)],
                EDITING_RATE_LIMIT: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_rate_limit_input)],
                EDITING_WELCOME: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_welcome_input)],
                EDITING_CLONE: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_clone_input)],
                EDITING_SUPPORT: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_support_input)],
            },
