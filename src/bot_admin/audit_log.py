...
class AuditLogger:
    ...
    def log(
        ...
        try:
            log = AuditLog(
                admin_id=admin_id, action=action, target=target, details=details, result=result, ip_address=ip_address
            )
            session.add(log)
            session.commit()

            logger.info(f"Audit: admin={admin_id}, action={action}, target={target}, result={result}")
