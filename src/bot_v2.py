...
setup_logging(level=log_level, json_format=log_format_json, log_file=log_file)
...
class TelegramBotV2:
    ...
    async def _register_standardized_modules(self):
        ...
        self.registry.register(menu_module, priority=0, enabled=True, metadata={"description": "主菜单和导航"})

        # 注册Premium模块
        from src.modules.premium.delivery import PremiumDeliveryService

        delivery_service = PremiumDeliveryService(bot=self.app.bot, order_manager=order_manager)
        ...
        self.registry.register(premium_module, priority=2, enabled=True, metadata={"description": "Premium会员功能"})

        # 注册能量模块 - 已修复，使用SafeConversationHandler
        energy_module = EnergyModule()
        self.registry.register(energy_module, priority=3, enabled=True, metadata={"description": "能量兑换功能"})

        # 注册地址查询模块 - 已修复，使用SafeConversationHandler
        address_query_module = AddressQueryModule()
        self.registry.register(address_query_module, priority=4, enabled=True, metadata={"description": "地址查询功能"})

        # 注册个人中心模块 - 标准化版本
        from src.modules.profile.handler import ProfileModule

        profile_module = ProfileModule()
        self.registry.register(profile_module, priority=5, enabled=True, metadata={"description": "个人中心和钱包"})

        # 注册TRX兑换模块 - 标准化版本
        from src.modules.trx_exchange.handler import TRXExchangeModule

        trx_module = TRXExchangeModule()
        self.registry.register(trx_module, priority=6, enabled=True, metadata={"description": "TRX闪兑"})

        # 注册管理员模块 - 标准化版本（包装现有AdminHandler）
        from src.modules.admin.handler import AdminModule

        admin_module = AdminModule()
        self.registry.register(admin_module, priority=10, enabled=True, metadata={"description": "管理员面板"})

        # 注册订单查询模块 - 标准化版本
        from src.modules.orders.handler import OrdersModule

        orders_module = OrdersModule()
        self.registry.register(orders_module, priority=11, enabled=True, metadata={"description": "订单查询"})

        # 注册健康检查模块 - 标准化版本
        from src.modules.health.handler import HealthModule

        health_module = HealthModule()
        self.registry.register(health_module, priority=1, enabled=True, metadata={"description": "健康检查"})

        # 注册帮助模块 - 标准化版本
        from src.modules.help.handler import HelpModule

        help_module = HelpModule()
        self.registry.register(help_module, priority=12, enabled=True, metadata={"description": "帮助中心"})
    ...
    async def _global_error_handler(self, update: object, context) -> None:
        ...
        try:
            if update and hasattr(update, "effective_user") and update.effective_user:
                ...
                error_message = "❌ <b>发生错误</b>\n\n抱歉，处理您的请求时出现问题。\n请稍后重试或联系客服。"
                await context.bot.send_message(chat_id=user_id, text=error_message, parse_mode="HTML")
    ...
    async def _setup_bot_commands(self):
        ...
        try:
            ...
            await self.app.bot.set_my_commands(common_commands, scope=BotCommandScopeDefault())
        ...
        if settings.bot_owner_id and settings.bot_owner_id > 0:
            try:
                ...
                await self.app.bot.set_my_commands(
                    admin_commands, scope=BotCommandScopeChat(chat_id=settings.bot_owner_id)
                )
    ...
    async def _init_scheduler(self):
        ...
        self.scheduler.add_job(
            run_energy_sync, "interval", minutes=5, id="energy_sync", name="能量订单状态同步", replace_existing=True
        )
    ...
    async def _check_production_config(self):
        ...
        if is_prod:
            ...
            if not api_keys:
                logger.warning("⚠️ PRODUCTION WARNING: API_KEYS not configured. API endpoints will reject all requests.")
            else:
            ...
            if not owner_id:
                logger.warning(
                    "⚠️ PRODUCTION WARNING: BOT_OWNER_ID not configured. Admin notifications will be disabled."
                )
            else:
