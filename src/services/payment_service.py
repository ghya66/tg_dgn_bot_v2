...
class PaymentService:
    ...
    def generate_unique_amount(self, base_amount: Decimal, order_id: str) -> tuple[Decimal | None, str | None]:
        """
        生成唯一金额（带后缀）
    ...
    def create_payment_order(
        self, user_id: int, order_type: str, amount: Decimal, related_data: dict[str, Any] | None = None
        ...
        try:
            order_id = order_manager.create_order(
                user_id=user_id, order_type=order_type, amount=float(amount), data=related_data
            )
    ...
    def confirm_payment(
        self, order_id: str, tx_hash: str | None = None, payment_method: str = "transfer"
    ) -> tuple[bool, str | None]:
        """
        确认支付
        ...
        """
        try:
            success = order_manager.confirm_payment(order_id=order_id, tx_hash=tx_hash, payment_method=payment_method)
