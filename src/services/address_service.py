...
from ..database import AddressQueryLog, SessionLocal
...
class AddressService:
    ...
    def validate_address(self, address: str) -> tuple[bool, str | None]:
        """
        验证 TRON 地址格式
    ...
    def check_rate_limit(self, user_id: int) -> tuple[bool, int]:
        ...
        try:
            db = SessionLocal()
            last_query = (
                db.query(AddressQueryLog)
                .filter_by(user_id=user_id)
                .order_by(AddressQueryLog.last_query_at.desc())
                .first()
            )
    ...
    def record_query(self, user_id: int, address: str) -> bool:
        ...
        try:
            ...
            if log:
                ...
                log = AddressQueryLog(user_id=user_id, address=address, last_query_at=datetime.now())
                db.add(log)
