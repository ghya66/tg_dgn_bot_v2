...
# 从本模块导入业务逻辑类
from .models import EnergyOrderType
from .states import *
...
class EnergyModule(BaseModule):
    ...
    async def _show_package_input(self, query, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        await query.edit_message_text(text=text, reply_markup=EnergyKeyboards.back_and_cancel(), parse_mode="HTML")
    ...
    async def input_usdt_amount(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            error_text = EnergyMessages.INVALID_AMOUNT.format(error_message=self.formatter.escape_html(str(e)))
            await message.reply_text(text=error_text, parse_mode="HTML")
            return STATE_INPUT_USDT
    ...
    async def _show_payment_info(self, message, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if energy_type == EnergyOrderType.HOURLY.value:
            ...
            payment_address = (
                settings.trx_receive_addr if hasattr(settings, "trx_receive_addr") else "TPaymentAddress123"
            )
        ...
        await message.reply_text(text=text, reply_markup=EnergyKeyboards.payment_done(), parse_mode="HTML")
    ...
    async def payment_done(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        await query.edit_message_text(text=text, reply_markup=EnergyKeyboards.skip_tx_hash(), parse_mode="HTML")
    ...
    async def handle_tx_hash_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # 显示确认消息
        text = EnergyMessages.ORDER_SUBMITTED.format(order_id=order_id, tx_hash=tx_hash)
