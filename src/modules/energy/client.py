...
class EnergyAPIClient:
    ...
    async def _request(self, endpoint: str, data: dict[str, Any], use_backup: bool = False) -> dict[str, Any]:
        """
        发送API请求
        ...
        # 添加认证信息
        request_data = {"username": self.username, "password": self.password, **data}
        ...
        try:
            ...
            response = await client.post(url, json=request_data, headers={"Content-Type": "application/json"})
    ...
    async def buy_energy(self, receive_address: str, energy_amount: int, rent_time: int = 1) -> APIOrderResponse:
        """
        为指定地址购买时长能量
        ...
        Returns:
            订单响应
        """
        data = {"re_type": "ENERGY", "re_address": receive_address, "re_value": energy_amount, "rent_time": rent_time}
    ...
    async def auto_buy_energy(self, receive_address: str, transfer_trx: float) -> APIOrderResponse:
        """
        自动计算并购买能量
        根据转入的TRX金额自动计算能量数量
        ...
        Returns:
            订单响应
        """
        data = {"re_address": receive_address, "transfer_trx": transfer_trx}
    ...
    async def recycle_energy(self, order_id: str) -> APIOrderResponse:
        ...
        return APIOrderResponse(code=result.get("code"), msg=result.get("msg"), data=result.get("data"))

    async def buy_package(self, receive_address: str) -> APIOrderResponse:
        """
        购买笔数能量套餐
    ...
    async def activate_address(self, target_address: str) -> APIOrderResponse:
        """
        激活地址
    ...
    async def query_logs(
        self, start_date: str | None = None, end_date: str | None = None, page: int = 1, page_size: int = 20
    ) -> dict[str, Any]:
        """
        查询账号日志
