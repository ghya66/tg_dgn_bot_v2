...
class PremiumDeliveryService:
    ...
    async def deliver_premium(
        self, order_id: str, buyer_id: int, recipient_username: str, recipient_id: int | None, premium_months: int
    ) -> dict[str, Any]:
        """
        自动发货 Premium
        ...
        try:
            ...
            if not recipient_id:
                ...
                if not recipient_id:
                    return await self._handle_failure(
                        order_id, buyer_id, f"无法获取用户 @{recipient_username} 的ID，请确认用户名正确且用户允许被搜索"
                    )
            ...
            if balance < stars_needed:
                return await self._handle_failure(
                    order_id, buyer_id, f"Bot Stars 余额不足（需要 {stars_needed}，当前 {balance}），请联系管理员充值"
                )
            ...
            return await self._handle_failure(order_id, buyer_id, f"系统错误: {e!s}")
    ...
    async def _mark_delivered(self, order_id: str, recipient_id: int):
        ...
        with get_db_context() as db:
            order = db.query(PremiumOrder).filter(PremiumOrder.order_id == order_id).first()
            if order:
    ...
    async def _handle_failure(self, order_id: str, buyer_id: int, error_msg: str) -> dict[str, Any]:
        ...
        return {"success": False, "message": error_msg, "order_id": order_id}
