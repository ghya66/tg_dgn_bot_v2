...
def error_handler(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if user:
                logger.info(f"用户 {user.id} ({user.username or user.first_name}) 调用 {func.__name__}")
            ...
            try:
                if update.message:
                    ...
                    await update.callback_query.edit_message_text(
                        error_msg, parse_mode="HTML", reply_markup=reply_markup
                    )
...
def log_action(action_name: str = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 记录操作开始
            logger.info(f"[ACTION_START] {name} - 用户: {user.id if user else 'Unknown'}")

            try:
                result = await func(*args, **kwargs)
                # 记录操作成功
                logger.info(f"[ACTION_SUCCESS] {name} - 用户: {user.id if user else 'Unknown'}")
                return result
            except Exception as e:
                # 记录操作失败
                logger.error(f"[ACTION_FAILED] {name} - 用户: {user.id if user else 'Unknown'} - 错误: {e}")
                raise
...
def require_private_chat(func: Callable) -> Callable:
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        if update.effective_chat.type != "private":
            await update.message.reply_text("⚠️ 此功能仅支持私聊使用。\n请私聊我后重试。")
            return ConversationHandler.END
