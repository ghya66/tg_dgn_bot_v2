...
class ErrorCollector:
    ...
    def collect(
        self, error_type: str, message: str, context: dict | None = None, exception: Exception | None = None
    ) -> None:
        """
        收集错误
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        """
        if not self.errors:
            return {"total": 0, "types": {}, "recent": [], "status": "healthy"}
    ...
    def get_errors_in_timerange(self, hours: int = 1) -> list[dict[str, Any]]:
        """
        获取指定时间范围内的错误
        ...
        return [error for error in self.errors if datetime.fromisoformat(error["timestamp"]) > cutoff_time]
    ...
    def clear_old_errors(self, days: int = 7) -> int:
        ...
        self.errors = [error for error in self.errors if datetime.fromisoformat(error["timestamp"]) > cutoff_time]
    ...
    def save_to_file(self, filename: str | None = None) -> bool:
        """
        同步保存错误到文件
...
def collect_error(
    error_type: str, message: str, context: dict | None = None, exception: Exception | None = None
) -> None:
    """
    便捷函数：收集错误到全局收集器
