    ...
    for attempt in range(retries + 1):
        try:
            ...
            if resp.status_code >= 500:
                logger.warning(f"服务器错误 {resp.status_code}，URL: {url}，尝试 {attempt + 1}/{retries + 1}")
            ...
            if attempt < retries:
                logger.warning(f"请求失败，将在 {retry_delay}s 后重试 (尝试 {attempt + 1}/{retries + 1}): {e}")
                await asyncio.sleep(retry_delay)
            else:
                logger.error(f"请求失败，已达最大重试次数 ({retries + 1}): {e}")
                raise
