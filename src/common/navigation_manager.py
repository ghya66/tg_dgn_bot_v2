...
class NavigationManager:
    ...
    async def handle_navigation(
        cls, update: Update, context: ContextTypes.DEFAULT_TYPE, target: str | None = None
    ) -> int:
        """
        处理导航请求
    ...
    def _cleanup_conversation_data(cls, context: ContextTypes.DEFAULT_TYPE):
        ...
        """
        # 保留必要的用户数据
        preserved_data = {k: v for k, v in context.user_data.items() if k in cls.PRESERVED_KEYS}
    ...
    @classmethod
    async def cleanup_and_show_main_menu(cls, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """
    ...
    async def _show_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query:
            await query.edit_message_text(
                "🔐 <b>管理员面板</b>\n\n请选择要执行的操作：", reply_markup=AdminMenus.main_menu(), parse_mode="HTML"
            )
    ...
    def create_back_button(
        cls, text: str = "🔙 返回主菜单", callback_data: str = "nav_back_to_main"
    ...
    @classmethod
    def standardize_keyboard(cls, keyboard: list, add_back_button: bool = True) -> list:
        """
        标准化键盘布局，确保有返回按钮
    ...
    @classmethod
    async def handle_fallback_callback(cls, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """
        ...
        if query:
            ...
            logger.warning(f"兜底处理器捕获未处理的回调: user={user.id}, data={callback_data}")
