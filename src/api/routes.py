...
async def get_statistics(_: str = Depends(api_key_auth)):
    """获取系统统计信息（需要认证）"""
    registry = get_registry()
    ...
    return {"success": True, "data": {"modules": module_stats, "orders": order_stats, "timestamp": datetime.now()}}
...
async def list_modules(enabled_only: bool = Query(False, description="只返回启用的模块")):
    """列出所有模块"""
    registry = get_registry()
    modules = []
...
async def update_module_status(module_name: str, request: ModuleStatusRequest, _: str = Depends(api_key_auth)):
    """启用或禁用模块（需要认证）"""
    registry = get_registry()
    ...
    return {"success": True, "message": f"Module {module_name} {'enabled' if request.enabled else 'disabled'}"}
...
async def check_premium_eligibility(user_id: int, _: str = Depends(api_key_auth)):
    """检查用户是否可以开通Premium"""
...
async def create_order(request: OrderCreateRequest, _: str = Depends(api_key_auth)):
    """创建订单（需要认证）"""
    try:
        order = await order_manager.create_order(request.user_id, request.base_amount)
...
async def get_order(order_id: str, _: str = Depends(api_key_auth)):
    """获取订单详情（需要认证）"""
...
async def get_user_balance(user_id: int, _: str = Depends(api_key_auth)):
    """获取用户余额（需要认证）"""
    wallet_manager = WalletManager()
    balance = await wallet_manager.get_balance(user_id)

    return UserBalanceResponse(user_id=user_id, balance=balance, currency="USDT", updated_at=datetime.now())


@router.post("/wallet/deposit", tags=["Wallet"])
async def add_balance(user_id: int, amount: float, reason: str = "API deposit", _: str = Depends(api_key_auth)):
    """增加用户余额（需要认证）"""
    wallet_manager = WalletManager()
    ...
    return {
        "success": True,
        "data": {"user_id": user_id, "amount_added": amount, "new_balance": new_balance, "reason": reason},
    }
...
async def send_message(request: SendMessageRequest, _: str = Depends(api_key_auth)):
    """发送消息给用户（需要认证）"""
...
async def broadcast_message(
    message: str, user_ids: list[int], parse_mode: str = "HTML", _: str = Depends(api_key_auth)
):
    """广播消息给多个用户（需要认证）"""
    from telegram import Bot
    ...
    for user_id in user_ids:
        try:
            await bot.send_message(chat_id=user_id, text=message, parse_mode=parse_mode, disable_notification=True)
            success_count += 1
        except Exception as e:
            logger.warning(f"Failed to send message to {user_id}: {e}")
            failed_count += 1

    return {"success": True, "data": {"total": len(user_ids), "success": success_count, "failed": failed_count}}
...
async def calculate_energy_price(energy_amount: int):
    """计算能量价格"""
    # 从标准化模块导入业务逻辑类
    from src.modules.energy.models import EnergyPriceConfig
    ...
    return {"success": True, "data": {"energy_amount": energy_amount, "price_trx": price}}
...
async def get_energy_account(_: str = Depends(api_key_auth)):
    """
...
async def buy_hourly_energy(request: EnergyBuyRequest, _: str = Depends(api_key_auth)):
    """
    购买时长能量（需要认证）
    ...
    try:
        ...
        if request.energy_amount not in [65000, 131000]:
            raise HTTPException(status_code=400, detail="energy_amount 必须是 65000 或 131000")

        client = get_energy_api_client()
        response = await client.buy_energy(
            receive_address=request.receive_address, energy_amount=request.energy_amount, rent_time=request.rent_time
        )
...
async def buy_energy_package(request: EnergyPackageBuyRequest, _: str = Depends(api_key_auth)):
    """
    购买笔数套餐（需要认证）
    ...
    try:
        client = get_energy_api_client()
        response = await client.buy_package(receive_address=request.receive_address)
...
async def query_energy_order(order_id: str, _: str = Depends(api_key_auth)):
    """
    查询能量订单状态（需要认证）
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"order_id": order_id, "code": response.code, "msg": response.msg, "details": response.data},
        }
    except Exception as e:
...
async def activate_address(request: EnergyActivateRequest, _: str = Depends(api_key_auth)):
    """
    激活波场地址（需要认证）
    ...
    try:
        client = get_energy_api_client()
        response = await client.activate_address(target_address=request.target_address)
